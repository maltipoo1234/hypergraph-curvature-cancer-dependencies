...
for fdr_thresh in [0.05, 0.1]:
    fdr = discovery[(discovery["fdr"] <= fdr_thresh) & (discovery["delta_dependency"] <= 0.0)].copy()
    ...
    nulls_m = matched_null_samples(str_sum_copy, "is_overlap_family", rng, iterations=10000, max_iterations=20000, metric="min_hlrc", features=features)
    
    if not nulls_m.empty:
        summ_m = summarize_nulls(nulls_m)
        print(f"FDR: {fdr_thresh}, n: {n}, p: {summ_m.iloc[0]['empirical_p_less']:.4f}")
